page 50003 "Payment Card"
{
        ...
        area(processing)
        {
            group(RequestApproval)
            {
                ...
                action(Approvals)
                {
                    ...
                    begin
                        //WorkflowsEntriesBuffer.RunWorkflowEntriesPage(RecordId, DATABASE::"Payment Header", DocType::Payment, Rec."No.");
                    end;
                }
                action("Unvote/Decommit Journal Voucher")
                {
                    Caption = 'Unvote/Decommit Journal Voucher';
                    Promoted = true;
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                        ...
                        if UserSetup.FindFirst then
                            if UserSetup.CommitBudget then
                                if Confirm('Are you sure you want to unvote this journal voucher') then
                                    FundsManagement.ReversePaymentsGLBudget(Rec);// FundsManagement.CommitjounalVoucher(Rec); //FundsApprovalManager.ReOpenPaymentHeader(Rec);
                        Message(Txt_004);
                        // CurrPage.CLOS
                        //END;
                    end;
                ...
                action("Vote Payment Voucher")
                {
                    Caption = 'Vote Payment Voucher';
                    Image = Planning;
                    Promoted = true;
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ...
                    begin
                        if UserSetup.Get(UserId) then begin
                            if not UserSetup.CommitBudget then
                                Error('You dont have a permission,Contact the Admin');
                            //ImprestLine.TESTFIELD("Net Amount");
                            //ImprestLine.TESTFIELD("Budget Gl");
                            if Confirm('Are you sure you want to Vote this Payment Voucher') then
                                FundsManagement.commitPaymentsGLBudget(Rec);
                            Message('Voted Successfully');//FundsManagement.CheckGLBudget(Rec);
                        end;
                    end;
                }
            ...
            group(Approval)
            {
                ...
                action(Delegate)
                {
                    ...
                    var
                        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                    begin
                        if Confirm('Are you sure you want to Delegate the Request?', true) then begin
    ...
    var
        ...
        Txt_004: Label 'Unvoted Successfully';
