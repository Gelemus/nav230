...
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Layouts/Payslips.rdl';
                ...
                begin
                    ...
                    if Today > Employee."Birth Date" then
                        if (Employee."Birth Date" <> 0D) then
                            Age := Dates.DetermineAge_Years(Employee."Birth Date", Today);
                    if Periods."Start Date" <> 0D then
                        Age := Dates.DetermineAge_Years(Employee."Birth Date", Periods."Start Date");
