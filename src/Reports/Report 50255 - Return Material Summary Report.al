report 50255 "Return Material Summary Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Layouts/Return Material Summary Report.rdl';

    dataset
    {
        dataitem("Store Requisition Header"; "Store Requisition Header")
        {
            DataItemTableView = SORTING("No.") ORDER(Ascending) WHERE("Return to store" = CONST(true), Status = CONST(Posted));
            RequestFilterFields = "No.", "Posting Date", "Requester ID", Status, "Posted By";
            ...
            column(Point_of_Use; "Store Requisition Header"."Point Of Use")
            {
            }
