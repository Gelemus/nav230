report 52225 "Portal P9A Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Layouts/Portal P9A Report.rdl';
    ...
    dataset
    {
        dataitem(Year; Year)
        {
            ...
            {
                RequestFilterFields = "No.", "Global Dimension 1 Code", "Global Dimension 2 Code", Status, "Statistics Group Code";
                ...
                column(Employee_s_____P_I_N_; gvPinNo)
                {
                }
                ...
                column(TOTAL_TAX__COL_M__KShs_Caption; TOTAL_TAX__COL_M__KShs_CaptionLbl)
                {
                }
                // column(DataItem464; DATE_OF_OCCUPATION_OF_HOUSE__________________________________________________________________________________________________Lbl)
                // {
                // }
                // column(DataItem466; L_R__NO__OF_OWNER_OCCUPIED_PROPERTY__________________________________________________________________________________________Lbl)
                // {
                // }
                ...
                // column(DataItem475; Photostat_copy_of_preceding_year_s_certificate_or_confirmation_of_current_year_s_borrowing_if_applicable_from_the_financial_iLbl)
                // {
                // }
                ...
                dataitem("Payroll Header"; "Payroll Header")
                {
                    ...
                    DataItemTableView = SORTING("Payroll Month") ORDER(Ascending) WHERE(Posted = CONST(true));

                    trigger OnPreDataItem()
                    begin
                        SetRange("Payroll Code", gvPayrollCode2);
                    end;
                }
                ...
                begin
                    ...
                    //EmployeePIN :=Employee."Payroll Code";
                    ...
                    if Header.Find('-') then begin
                        repeat
                            ...
                            if Header."L (LCY)" > 0 then begin  //SKM130208 OC-ES037 do not show negative PAYE
                    ...
                    if ((AmountArray[1] [13] = 0) and
                     ...
                     (AmountArray[13] [13] = 0) and
                     (AmountArray[14] [13] = 0)) then
                        CurrReport.Skip;
                    //END skip blank P9s
                ...
                begin
                    ...
                    if gvEmployeeNoFilter <> '' then
                        SetRange("No.", gvEmployeeNoFilter);

                    YearText := 'INCOME TAX DEDUCTION CARD YEAR ' + Format(Year.Year);

                    PayrollSetupRec.Get(gvPayrollCode2);
                    ...
                    Header.SetRange("Payroll Code", gvPayrollCode2);
            ...
            trigger OnPreDataItem()
            begin
                if gvPeriodIDFilter <> 0 then
                    SetRange(Year, gvPeriodIDFilter);
            end;
        }
    }
    ...
    begin
        ...
        if gvEmployeeNoFilter = '' then
            gvEmployeeNoFilter := Employee.GetFilter("No.");//ICS APR2

        if gvEmployeeNoFilter <> '' then
            if EmployeeRec2.Get(gvEmployeeNoFilter) then
                gvPayrollCode2 := EmployeeRec2."Payroll Code";
        PayrollSetupRec.Get(gvPayrollCode2);
    end;
    ...
    var
        ...
        DateCommenced: Date;
        DateTerminated: Date;
        OldEmployer: Text[100];
        OldAddr: Text[100];
        NewEmployer: Text[100];
        NewAddr: Text[100];
        MonthlyRent: Decimal;
        ComAddr: Record "Company Information";
        ...
        TOTAL_TAX__COL_M__KShs_CaptionLbl: Label 'TOTAL TAX (COL. M) KShs.';
        //   DATE_OF_OCCUPATION_OF_HOUSE__________________________________________________________________________________________________Lbl: Label 'DATE OF OCCUPATION OF HOUSE:............................................................................................................';
        //  L_R__NO__OF_OWNER_OCCUPIED_PROPERTY__________________________________________________________________________________________Lbl: Label 'L.R. NO. OF OWNER OCCUPIED PROPERTY:........................................................................................................';
        ...
        ///  Photostat_copy_of_preceding_year_s_certificate_or_confirmation_of_current_year_s_borrowing_if_applicable_from_the_financial_iLbl: Label 'Photostat copy of preceding year''s certificate or confirmation of\current year''s borrowing if applicable from the financial institution.';
        LCaptionLbl: Label ' L';
        KCaptionLbl: Label ' K';
        JCaptionLbl: Label ' J';
        HCaptionLbl: Label ' H';
        ...
        gvEmployeeNoFilter: Code[50];
        EmployeeRec2: Record Employee;
        gvPayrollCode2: Code[50];
        gvPeriodIDFilter: Integer;
