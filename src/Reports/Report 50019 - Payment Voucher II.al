...
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/Layouts/Payment Voucher II.rdl';
    ...
    dataset
    {
        dataitem("Payment Header"; "Payment Header")
        {
            ...
            {
                DataItemLink = "Document No." = FIELD("No.");
                DataItemTableView = WHERE(Status = FILTER(Open | Created | Approved));
                ...
                begin
                    ...
                    if i = 1 then begin
                        ...
                        if EmployeeRecI.FindFirst then begin
                            PreparedByCaption := 'Prepared by Payable Accountant: ';//+' '+EmployeeRecI."Job Title";//+' '+EmployeeRecI."Last Name";
                    ...
                    if i = 2 then begin
                        ApprovedByCaption := 'Approved by Fiance Manager: ';
                        UserSetupRec.Reset;
                        UserSetupRec.SetRange("User ID", "Approval Entry"."Approver ID");
                        if UserSetupRec.FindFirst then
                            UserSetupRec.CalcFields(Signature);
